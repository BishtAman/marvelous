...
import CHARACTERS from "../../data/character-name.js";
  ...
  return (
    <div>
      <section className="flex justify-center items-center bg-[#87b3a6d9] mb-[60px] mt-[20px] h-[80px]">
        <button
          className="steir teirs"
          onClick={() => setCurrentTeir("S Tier")}
        >
          <PageHeading heading={S} className="justify-start " />
        </button>
        <button
          className="ateir teirs"
          onClick={() => setCurrentTeir("A Tier")}
        >
          <PageHeading heading={A} className="justify-start " />
        </button>
        <button
          className="bteir teirs"
          onClick={() => setCurrentTeir("B Tier")}
        >
          <PageHeading heading={B} className="justify-start " />
        </button>
        <button
          className="cteir teirs"
          onClick={() => setCurrentTeir("C Tier")}
        >
          <PageHeading heading={C} className="justify-start " />
        </button>
        <button
          className="dteir teirs"
          onClick={() => setCurrentTeir("D Tier")}
        >
      ...
      <div className="flex flex-wrap  justify-evenly ">
        {loadedCharacters.map(({ id, name, imageURL }) => (
          <CharacterCard key={id} name={name} imageURL={imageURL} />
        ))}
      </div>
    </div>
  );
